...
def create_audit_log(db: Session, campaign_id: int, user_id: int, action: str):
    log = CampaignAuditLog(campaign_id=campaign_id, user_id=user_id, action=action)

    db.add(log)


def get_campaign_member(db: Session, campaign_id: int, user_id: int):
    member = (
...
def get_task(db: Session, task_id: int):
    ...
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Campaign task not found"
        )

    return task


def validate_assignee(db: Session, campaign_id: int, assignee_id: int | None):
...
def create_campaign_task(
    db: Session, campaign_id: int, user_id: int, task_data: CampaignTaskCreate
):
    get_campaign(db, campaign_id)

    get_campaign_member(db, campaign_id, user_id)

    validate_assignee(db, campaign_id, task_data.assignee_id)
    ...
    create_audit_log(db, campaign_id, user_id, "CREATE_TASK")
...
def get_campaign_task(db: Session, task_id: int, user_id: int):
    task = get_task(db, task_id)

    get_campaign_member(db, task.campaign_id, user_id)

    return task


def update_campaign_task(
    db: Session, task_id: int, user_id: int, task_data: CampaignTaskUpdate
):
    task = get_task(db, task_id)

    member = get_campaign_member(db, task.campaign_id, user_id)
    ...
    if not is_owner:
        if "title" in update_data or "description" in update_data:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only the campaign owner can update title or description",
            )
    ...
    if "assignee_id" in update_data:
        validate_assignee(db, task.campaign_id, update_data["assignee_id"])

    for field, value in update_data.items():
        setattr(task, field, value)

    create_audit_log(db, task.campaign_id, user_id, "UPDATE_TASK")
...
def delete_campaign_task(db: Session, task_id: int, user_id: int):
    task = get_task(db, task_id)
    ...
    create_audit_log(db, task.campaign_id, user_id, "DELETE_TASK")
