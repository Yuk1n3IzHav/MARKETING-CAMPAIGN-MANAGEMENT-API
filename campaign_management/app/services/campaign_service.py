...
def _check_owner(db: Session, campaign: Campaign, user: User):
...
def create_campaign(db: Session, current_user: User, data: CampaignCreate):
    ...
    campaign = Campaign(
        name=name, description=data.description, owner_id=current_user.id
    )
    ...
    owner_member = CampaignMember(
        campaign_id=campaign.id, user_id=current_user.id, role="OWNER"
    )
...
def get_campaigns(db: Session, current_user: User, search: str | None = None):
    query = (
        ...
        .filter(CampaignMember.user_id == current_user.id, Campaign.is_deleted == False)
    )
...
def get_campaign_detail(db: Session, campaign_id: int, current_user: User):
...
def update_campaign(
    db: Session, campaign_id: int, current_user: User, data: CampaignUpdate
):
    campaign = _get_campaign(db, campaign_id)

    _check_owner(db, campaign, current_user)
    ...
    _create_audit_log(db, campaign.id, current_user.id, "UPDATE_CAMPAIGN")
...
def delete_campaign(db: Session, campaign_id: int, current_user: User):
    ...
    _create_audit_log(db, campaign.id, current_user.id, "DELETE_CAMPAIGN")
